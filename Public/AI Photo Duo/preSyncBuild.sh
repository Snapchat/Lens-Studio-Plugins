...
# ============================================================================
# preSyncBuild.sh -- AI-Photo-Duo CDS sync build script
#
# Steps:
#   1. npm install + npm run build (compile TypeScript to dist/)
#   2. Package: create "AI Photo Duo/" zip structure (flattened)
#   3. Place zip in build/resources/<version>/AI Photo Duo.zip
#
# NAMING NOTE:
#   The plugin DIRECTORY is "AI-Photo-Duo" (hyphen) but the shipped MODULE
#   FOLDER and zip file use "AI Photo Duo" (space), matching the original
#   module.json name.
#
# STAGING LAYOUT:
#   AI Photo Duo uses a FLATTENED single-folder layout.
...
#           module.json
#
#   Zip filename: "AI Photo Duo.zip"
# ============================================================================

echo "=== AI-Photo-Duo: preSyncBuild.sh starting ==="
...
# --------------------------------------------------------------------------
echo "--- Step 2: Creating AI Photo Duo.zip ---"
...
STAGE_DIR="${TMPDIR:-/tmp}/ai-photo-duo-pkg-$$"
MODULE_DIR="$STAGE_DIR/AI Photo Duo"
...
ZIP_DEST="$ZIP_DIR/AI Photo Duo.zip"
(cd "$STAGE_DIR" && zip -r "$ZIP_DEST" "AI Photo Duo")

echo "Created: $ZIP_DEST"

echo "=== AI-Photo-Duo: preSyncBuild.sh completed ==="
