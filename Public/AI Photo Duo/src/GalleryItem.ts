...
export class GalleryItem {
    ...
    constructor(parent: Ui.Widget, id: string) {
        ...
        [this.frame, this.border].forEach((item) => {
    ...
    private removeIfPossible(path: Editor.Path): void {
        try {
            ...
            // Pixmap may still hold the file open, which on Windows means an
            // exclusive lock - leave it for the TempDir teardown.
            console.error(`[AI Photo Duo] GalleryItem could not remove superseded preview "${path.toString()}":`, e, console.None);
        }
    }
    ...
    addPreview(previewUrl: string) {
        ...
        downloadFile(previewUrl, (response: any) => {
            ...
            if (response.statusCode !== 200) {
                console.error(`[AI Photo Duo] GalleryItem.addPreview failed for "${this.id}", status code: ${response.statusCode}`);
                return;
            }
            const bytes = response.body.toBytes();
            if (!bytes || bytes.length === 0) {
                console.error(`[AI Photo Duo] GalleryItem.addPreview "${this.id}" got an empty body from the preview URL`);
                return;
            }

            // A tile can be given a preview more than once, and the file already on
            // disk may still be open by the Pixmap. Windows refuses to delete open
            // files, so write each preview under a fresh name rather than
            // overwriting in place.
            const uniqueName = `${this.id}_preview_${Date.now().toString(36)}_${GalleryItem.uniqueCounter++}`;
            const path = tempDir.path.appended(new Editor.Path(uniqueName + ".jpeg"));
            ...
            if (resolvedFilePath.startsWith(resolvedDirectoryPath)) {
                try {
                    ...
                    // Now that the view points at the new file, the old one may be
                    // unlocked - drop it so temp usage doesn't grow with refreshes.
                    if (supersededPath) {
                        this.removeIfPossible(supersededPath);
                    }
                } catch (e) {
                    console.error(`[AI Photo Duo] GalleryItem.addPreview failed to save/load preview "${this.id}":`, e);
                }
            }
            else {
    ...
    /**
     * Destroys the tile widget. Must be called when the gallery resets -
     * simply dropping the JS reference can leave native widgets resident.
     */
    dispose(): void {
        if (this.disposed) {
            return;
