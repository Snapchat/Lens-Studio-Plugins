...
export class EffectGallery {
    ...
    private createEmptyGalleryPage(parent: Ui.Widget): Ui.Widget {
        ...
        title.text = '<center>Welcome to<br><span style="font-size: 16px; font-weight: bold; color: #FFF0B9;">AI Video Transform</span><center>';
        title.wordWrap = true;
        ...
        this.connections.push(guidelinesButton.onClick.connect(() => {
            Shell.openUrl('https://developers.snap.com/lens-studio/features/genai-suite/ai-video-transform', {});
        }));
    ...
    private getDreamsGallery() {
        ...
        const filters = {search: this.searchText, favoritesOnly: this.favoritesOnly};
        listDreams(this.INITIAL_PAGE_SIZE, (response: any) => {
            if (currentRequestId !== this.lastRequestId) {
                return;
            }
            if (response.statusCode !== 200) {
                console.error(`[AI Video Transform] listDreams failed, status code: ${response.statusCode}`);
                return;
            }
            ...
            if (body.items.length > 0 && this.stackedWidget) {
                ...
                if (this.searchText || this.favoritesOnly) {
                    ...
                    this.stackedWidget.currentIndex = 1;
                    this.gallery.showEmpty(this.favoritesOnly ? 'No favorite effects yet.' : 'No effects found.');
                } else {
    ...
    private expandGallery() {
        ...
        listDreams(this.EXPAND_PAGE_SIZE, (response: any) => {
            ...
            if (response.statusCode !== 200) {
                console.error(`[AI Video Transform] listDreams (expand) failed, status code: ${response.statusCode}`);
                this.isPrefetching = false;
                return;
            }
    ...
    private prefetchNextPage() {
        ...
        listDreams(this.EXPAND_PAGE_SIZE, (response: any) => {
            ...
            if (response.statusCode !== 200) {
                console.error(`[AI Video Transform] listDreams (prefetch) failed, status code: ${response.statusCode}`);
                this.isPrefetching = false;
                return;
            }
    ...
    private onTileClicked(id: string, callback: Function) {
        if (this.settings[id]) {
            ...
            getDreamByID(id, (response: any) => {
                callback();
                if (response.statusCode !== 200) {
                    console.error(`[AI Video Transform] getDreamByID failed, status code: ${response.statusCode}`);
                    return;
                }
