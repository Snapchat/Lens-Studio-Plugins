...
export class EffectSettings {
    ...
    create(parent: Ui.Widget, effectGalleryWidget: Ui.Widget): Ui.Widget {
        ...
        warningLabel.text = "Do not add music, the Music Lyrics component, or the Face Animator component to your project. These components are not compatible with AI Video Transform and will prevent the Lens from being published.";
        ...
        return new Promise((resolve) => {
            getDreamByID(id, (response: any) => {
                if (response.statusCode == 200) {
                    ...
                    console.error(`[AI Video Transform] importById/getDreamByID failed, status code: ${response.statusCode}`);
                    resolve();
                }
            });
        });
    ...
    private previewEffect(): void {
        ...
        createDream(prompt, seed, (response: any) => {
            if (response.statusCode == 200) {
                ...
                console.error(`[AI Video Transform] createDream failed, status code: ${response.statusCode}`);
    ...
    private createDreamPack() {
        createPack(this.curId, (response: any) => {
            if (response.statusCode == 200) {
                ...
                console.error(`[AI Video Transform] createPack rejected because dream ${this.curId} is already packed - refreshing state`);
                ...
                console.error(`[AI Video Transform] createPack failed, status code: ${response.statusCode}`);
                logEventEffectTraining("FAILED");
            }
        })
    }
