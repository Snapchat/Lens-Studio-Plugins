...
export class EffectSettingsPage {
    ...
    create(parent: Ui.Widget): Ui.Widget {
        ...
        const guidelinesLink = 'https://developers.snap.com/lens-studio/features/genai-suite/ai-video-transform';
        ...
        const previewLimitBanner = this.createCalloutWidget(
            widget,
            'The full experience is not available in Lens Studio Pro. Instead of the generated video, you will see a preview. For the full experience, use the Preview Lens function.'
        );
        layout.addWidgetWithStretch(previewLimitBanner, 0, Ui.Alignment.AlignBottom);
    ...
    private createLensPlusCard(parent: Ui.Widget): Ui.Widget {
        ...
        widget.pixmap = new Ui.Pixmap(new Editor.Path(import.meta.resolve('./Resources/lens_plus_card.svg')));
        widget.radius = 8;
        widget.scaledContents = true;

        const snapchatLogo = new Ui.ImageView(widget);
        snapchatLogo.pixmap = new Ui.Pixmap(new Editor.Path(import.meta.resolve('./Resources/snapchat_plus_logo.svg')));
        ...
        description.text = `Lenses created with "AI Video Transform" will be available only to Lens+ subscribers. Using "AI Video Transform" does not automatically enroll you in Lens+ Payouts, to begin monetizing your Lenses, ${lensPlusUrlString}.`;
    ...
    private createHeader(parent: Ui.Widget, text: string, onReturnClicked: Function): Ui.Widget {
        ...
        trashCanImageView.pixmap = new Ui.Pixmap(new Editor.Path(import.meta.resolve('./Resources/trashCan.svg')));
