...
export class Gallery {
    ...
    private emptyLabel: Ui.Label | undefined;
    ...
    create(parent: Ui.Widget): Ui.Widget {
        ...
        const emptyLabel = new Ui.Label(gridWidget);
        emptyLabel.text = '<center>No effects found.</center>';
        emptyLabel.foregroundRole = Ui.ColorRole.PlaceholderText;
        emptyLabel.wordWrap = true;
        emptyLabel.setFixedWidth(300);
        emptyLabel.move(45, 240);
    ...
    /**
     * Removes a tile from the grid entirely - used when a dream transitions
     * to a failed state while its tile is already shown (failed effects
     * aren't displayed in the gallery at all).
     */
    removeItem(id: string) {
        const item = this.pendingPreviews[id];
        ...
        if (index !== -1) {
            ...
            item.dispose();
            this.rebuildLayout();
            // rebuildLayout() clears and re-adds the grid layout, which (unlike
            // appendToLayout() called from addItems()) doesn't restore visibility
            // on its own - re-show the remaining tiles or they'd all disappear.
            this.items.forEach((remainingItem) => {
                remainingItem.widget.visible = true;
            });
        }
    ...
    reset() {
        // Destroy tiles (and their MovieView/VideoView decoded media) instead of
        // only hiding them - otherwise each gallery refresh leaks native RAM.
    ...
    /**
     * Stops the loading spinner and shows a "no effects" placeholder within
     * the gallery grid, while keeping the header (search/filter) visible -
     * used when a filter (e.g. favorites) matches zero effects.
     */
    showEmpty(message: string = 'No effects found.') {
        this.spinner.visible = false;
        if (this.emptyLabel) {
            this.emptyLabel.text = `<center>${message}</center>`;
