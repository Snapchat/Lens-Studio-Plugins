...
export class Importer {
    ...
    private async importEffect(filePath: any, packId: string, previewsPath: string[]) {
        ...
        const videoPreviewImportResult = await assetManager.importExternalFileAsync(previewsPath[0], new Editor.Path('./AI Video Transform Resources/'), Editor.Model.ResultType.Auto);
        const aiOutputRenderTarget = assetManager.createNativeAsset('RenderTarget', 'AI Output', new Editor.Path(`./AI Video Transform Resources/`));
        ...
        if (addScreenRegion) {
            ...
            componentSo.name = "AI Video Transform";
    ...
    importToProject(packId: string, previewsPath: string[]): Promise<any> {
        return this.importEffect(new Editor.Path(import.meta.resolve('./Resources/AI Video Transform.lsc')), packId, previewsPath);
    }
}
