...
export class PreviewDownloader {
     ...
     * is reopened), but the file already on disk may still be open by a
     * VideoView/Movie. Windows keeps an exclusive lock on open files, so
     * deleting and rewriting in place fails - write to a fresh name instead.
     */
    ...
    private removeIfPossible(path: Editor.Path): void {
        try {
            ...
            console.error(`[AI Video Transform] Could not remove superseded preview "${path.toString()}":`, e, console.None);
        }
    }
    ...
    download(url: string, fileName: string, callback: Function, extractFile: string = "preview.webp"): void {
        ...
        downloadFile(url, (response: any) => {
            if (response.error) {
                console.error(`[AI Video Transform] downloadFile "${fileName}" reported an error despite statusCode ${response.statusCode}: ${response.error}`);
            }
            if (response.statusCode === 200) {
                ...
                if (!bytes || bytes.length === 0) {
                    console.error(`[AI Video Transform] Downloaded preview "${fileName}" has an empty body - not attempting to save it.`);
                    return;
                }

                // The dreams API serves some previews as a direct asset (e.g. "video/mp4")
                // rather than a zip archive - detect that case and save the bytes as-is
                // instead of trying (and failing) to unpack them as a zip.
                ...
                if (!isZip) {
                    ...
                    try {
                        ...
                        console.error(`[AI Video Transform] Failed to save direct preview "${fileName}" (contentType: ${response.contentType}):`, e);
                    }
                    return;
                }

                if (!isContainedRelativePath(extractFile)) {
                    console.error(`[AI Video Transform] Refusing to unpack preview "${fileName}": requested entry "${extractFile}" is not a contained relative path.`);
                    return;
                }
                ...
                if (!entryCheck.contained) {
                    console.error(`[AI Video Transform] Refusing to unpack preview "${fileName}": ${entryCheck.reason}.`);
                    return;
                }
                ...
                const attempt = (attemptIndex: number) => {
                    ...
                    try {
                        ...
                        if (!resolvedExtractPath.startsWith(resolvedUnzipDir)) {
                            console.error(`[AI Video Transform] Unpacked preview "${fileName}" resolves outside its unpack directory - discarding it.`);
                        ...
                        if (attemptIndex < retryDelaysMs.length) {
                            console.error(`[AI Video Transform] Attempt ${attemptIndex + 1} failed to download/unpack preview "${fileName}", retrying in ${retryDelaysMs[attemptIndex]}ms:`, e);
                            ...
                            console.error(`[AI Video Transform] Failed to download/unpack preview "${fileName}" after ${attemptIndex + 1} attempts:`, e);
                        }
                    }
                };
                ...
                console.error(`[AI Video Transform] Failed to download preview "${fileName}", status code: ${response.statusCode}`);
            }
        })
    }
}
