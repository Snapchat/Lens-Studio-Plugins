...
export class EffectGallery {
    ...
    private readonly VISIBLE_PAGE_SIZE = 9;
    ...
    private createEmptyGalleryPage(parent) {
        ...
        title.text = '<center>Welcome to<br><span style="font-size: 16px; font-weight: bold; color: #FFF0B9;">AI Clips</span><center>';
        title.wordWrap = true;
        ...
        guidelinesButton.onClick.connect(() => {
            Shell.openUrl('https://developers.snap.com/lens-studio/features/genai-suite/ai-clips', {});
        })
    ...
    private processItems(items: any[]) {
        const batchData: Array<{id: string, description: string, previewUrl: string, isDefault?: boolean, inProgress?: boolean, isTraining?: boolean, isTrained?: boolean}> = [];
