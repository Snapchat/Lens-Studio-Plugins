...
# ============================================================================
# preSyncBuild.sh -- AI-Clips CDS sync build script
#
# Steps:
#   1. npm install + npm run build (compile TypeScript to dist/)
#   2. Package: create "AI Clips/" zip structure (flattened)
#   3. Place zip in build/resources/<version>/AI Clips.zip
#
# NAMING NOTE:
#   The plugin DIRECTORY is "AI-Clips" (hyphen) but the shipped MODULE FOLDER
#   and zip file use "AI Clips" (space), matching the original module.json name.
#
# STAGING LAYOUT:
#   AI Clips uses a FLATTENED single-folder layout.
...
#           module.json
#
#   Zip filename: "AI Clips.zip"
# ============================================================================

echo "=== AI-Clips: preSyncBuild.sh starting ==="
...
# --------------------------------------------------------------------------
echo "--- Step 2: Creating AI Clips.zip ---"
...
STAGE_DIR="${TMPDIR:-/tmp}/ai-clips-pkg-$$"
MODULE_DIR="$STAGE_DIR/AI Clips"
...
ZIP_DEST="$ZIP_DIR/AI Clips.zip"
(cd "$STAGE_DIR" && zip -r "$ZIP_DEST" "AI Clips")

echo "Created: $ZIP_DEST"

echo "=== AI-Clips: preSyncBuild.sh completed ==="
