...
function deserializeProperty(file, blobs, strings) {
    const type = file.readUint16();
    if (type === 0)
        return null; // end of collection
...
function deserializeString(file, strings) {
...
function deserializeBlob(file, blobs) {
...
function deserializeGroup(file, blobs, strings) {
    let obj = null;
    while (true) {
...
(function (lc) {
    ...
    let Topology;
    (function (Topology) {
        Topology[Topology["TRIANGLES"] = 0] = "TRIANGLES";
        Topology[Topology["TRIANGLE_STRIP"] = 1] = "TRIANGLE_STRIP";
        Topology[Topology["TRIANGLE_FAN"] = 2] = "TRIANGLE_FAN";
        Topology[Topology["POINTS"] = 3] = "POINTS";
        Topology[Topology["LINES"] = 4] = "LINES";
        Topology[Topology["LINE_STRIP"] = 5] = "LINE_STRIP";
    })(Topology = lc.Topology || (lc.Topology = {}));
    /** index buffer data type */
    let IndexType;
    (function (IndexType) {
        IndexType[IndexType["NONE"] = 0] = "NONE";
        IndexType[IndexType["USHORT"] = 1] = "USHORT";
        IndexType[IndexType["UINT"] = 2] = "UINT";
    })(IndexType = lc.IndexType || (lc.IndexType = {}));
    /** attribute types for vertex layout */
    let ComponentType;
    (function (ComponentType) {
        ComponentType[ComponentType["UNKNOWN"] = 0] = "UNKNOWN";
        ComponentType[ComponentType["BYTE"] = 1] = "BYTE";
        ComponentType[ComponentType["UBYTE"] = 2] = "UBYTE";
        ComponentType[ComponentType["SHORT"] = 3] = "SHORT";
        ComponentType[ComponentType["USHORT"] = 4] = "USHORT";
        ComponentType[ComponentType["FLOAT"] = 5] = "FLOAT";
        ComponentType[ComponentType["HALF"] = 6] = "HALF";
    })(ComponentType = lc.ComponentType || (lc.ComponentType = {}));
    /** returns the size in bytes of an index buffer element */
    function indexSize(type) {
        switch (type) {
    ...
    lc.componentSize = componentSize;
    ...
    (function (Semantic) {
        ...
        Semantic["Texture0"] = "texture0";
        Semantic["Texture1"] = "texture1";
        Semantic["Texture2"] = "texture2";
        Semantic["Texture3"] = "texture3";
        Semantic["BoneData"] = "boneData";
        Semantic["Joints"] = "joints";
        Semantic["Weights"] = "weights";
    })(Semantic = lc.Semantic || (lc.Semantic = {}));
})(lc || (lc = {}));
/** extract interleaved data into a compact buffer */
function arrayGather(buffer, offset, length, stride) {
...
class Attribute {
    constructor(node, inputBuffer, vertexStride) {
...
class VertexLayout {
    constructor(node, interleavedBuffer) {
        ...
        if (index !== -1) {
            ...
            const joints = new Attribute(new Map([
            ...
            const weights = new Attribute(new Map([
...
class BlendShape {
    constructor(node, basisNormalBuffer) {
...
BlendShape.PACKED_VERTEX_SIZE = 9;
BlendShape.VEC3_SIZE = 12;
/** not yet suported */
class VertexCache {
    constructor(node) {
    }
...
function gltfTopology(topology) {
    switch (topology) {
...
function gltfIndexType(indexType) {
    switch (indexType) {
...
function gltfComponentType(componentType) {
...
function gltfAccessorType(componentCount) {
...
function gltfSemantic(semantic) {
    switch (semantic) {
...
function generateGltf(mesh, assetName) {
    let g = {
        asset: {
    ...
    g.accessors = [];
    // vertex buffers
    const vertexAccessorIds = {};
    for (const attribute of mesh.attributes) {
        if (attribute.semantic === lc.Semantic.Bitangent)
...
function serializeGlb(pojo, binary) {
...
export function convertMeshFile(assetName, inputPath, outputPath) {
...
function debugDump(obj) {
    Reflect.set(vec2.prototype, "toJSON", function () {
        return { x: this.x, y: this.y };
    });
    Reflect.set(vec3.prototype, "toJSON", function () {
        return { x: this.x, y: this.y, z: this.z };
    });
    Reflect.set(vec4.prototype, "toJSON", function () {
        return { x: this.x, y: this.y, z: this.z, w: this.w };
    });
    Reflect.set(quat.prototype, "toJSON", function () {
        return { x: this.x, y: this.y, z: this.z, w: this.w };
    });
    Reflect.set(mat2.prototype, "toJSON", function () {
        return { c0: this.column0, c1: this.column1 };
    });
    Reflect.set(mat3.prototype, "toJSON", function () {
        return { c0: this.column0, c1: this.column1, c2: this.column2 };
    });
    Reflect.set(mat4.prototype, "toJSON", function () {
        return { c0: this.column0, c1: this.column1, c2: this.column2, c3: this.column3 };
    });
    Reflect.set(Map.prototype, "toJSON", function () {
        return Object.fromEntries(this.entries());
    });
    console.debug(JSON.stringify(obj, (_, val) => {
        if (ArrayBuffer.isView(val))
            return `ArrayBufferView(${val.byteLength})`;
