...
import { retextureAsset, createAsset, getAsset } from '../api.js';
...
export class PreviewMenu {
    ...
    modifyAsset(controls) {
        ...
        if (this.updateAssetOption.value == 'Change Texture') {
            retextureAsset(this.asset_id, buildAssetData(controls, false), (retextureResponse) => {
                ...
                } else if (retextureResponse.statusCode == 422) {
                    logEventAssetCreation("GUIDELINES_VIOLATION", "CHANGE_TEXTURE", inputFormat);
                    app.log('The result violates our community guidelines');
                    this.updateEditButtonVisibility();
                } else {
                    logEventAssetCreation("FAILED", "CHANGE_TEXTURE", inputFormat);
                    app.log('Something went wrong, please try again.');
                    this.updateEditButtonVisibility();
                }
            });
        } else if (this.updateAssetOption.value == 'Change Geometry') {
            ...
            createAsset(settings, (response) => {
                ...
                if (response.statusCode == 200) {
                    ...
                    logEventAssetCreation("GUIDELINES_VIOLATION", "CHANGE_GEOMETRY", inputFormat);
                    app.log('The result violates our community guidelines');
                    this.updateEditButtonVisibility();
                } else {
                    logEventAssetCreation("FAILED", "CHANGE_GEOMETRY", inputFormat);
                    app.log('Something went wrong, please try again.');
                    this.updateEditButtonVisibility();
                }
            });
    ...
    updatePreview(state) {
        ...
        getAsset(state.asset_id, (asset) => {
            ...
            if (asset && asset.attachment) {
                this.controls['imageReferencePicker'].value = [asset.attachment];
            }
        });

        this.updateAssetOption.value = 'Change Geometry';
    ...
    createHeader(parent) {
        ...
        this.header.setSizePolicy(Ui.SizePolicy.Policy.Expanding, Ui.SizePolicy.Policy.Fixed);
        ...
        this.headerTitle.text = '3D Asset';
    ...
    updateEditButtonVisibility() {
        if (this.updateAssetOption.value === 'Change Geometry') {
            ...
            // this.controls['imageReferencePicker'].widget.visible = true;
        } else if (this.updateAssetOption.value === 'Change Texture') {
            this.editEffectButton.enabled = ((this.status == 'SUCCESS') && this.controls['promptPicker'].value.length > 0);

            // this.controls['imageReferencePicker'].widget.visible = false;
        }
    }
