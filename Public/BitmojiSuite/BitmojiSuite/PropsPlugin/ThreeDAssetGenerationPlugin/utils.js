...
const guidelinesLink = 'https://developers.snap.com/lens-studio/features/bitmoji-suite/props';
...
function createCalloutWidget(parent, text, link) {
    ...
    frame.setSizePolicy(Ui.SizePolicy.Policy.Expanding, Ui.SizePolicy.Policy.Fixed)
...
export function createGenerationErrorWidget(parent) {
    ...
    disclaimerLabel.text = '<center>There was a problem generating the 3D asset.</center><center>Please try to generate another one.</center>';
...
export function importToProject(objectUrl, callback, file_format) {
    downloadFileFromBucket(objectUrl, file_format === "lsc" ? "Generative 3D Component.lsc" : "Generative 3D Package.lspkg", function(file_path, tempDir) {
        importEffect(file_path, tempDir, file_format).then(() => {
            callback(true);
        })
            .catch((error) => {
