...
export class Menu {
    ...
    private menuStackedWidget: Ui.StackedWidget | undefined;
    ...
    createMenu(parent: Ui.Widget): Widget {
        const widget = new Widget(parent);
        widget.setSizePolicy(Ui.SizePolicy.Policy.Expanding, Ui.SizePolicy.Policy.Expanding);
        ...
        const guidelinesLink = 'https://developers.snap.com/lens-studio/features/bitmoji-suite/animate';
    ...
    createButton(parent: Ui.Widget, iconName: string, hoveredIconName: string, size: number, idx: number, onButtonClicked: Function): Ui.Widget {
        const defaultImage = new Ui.Pixmap(new Editor.Path(import.meta.resolve('./Resources/' + iconName)));
        const hoveredImage = new Ui.Pixmap(new Editor.Path(import.meta.resolve('./Resources/' + hoveredIconName)));
