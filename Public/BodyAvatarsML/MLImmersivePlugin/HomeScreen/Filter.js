...
export class Filter {
    ...
    isDefault() {
        return this.actions[ALL_FILTER_ID][ALL_FILTER_ID].checked;
    }
    ...
    onModelUpdated(sendCallbackFlag) {
        this.updateView();
        if (sendCallbackFlag && !this.signalsBlocked) {
    ...
    toString() {
        ...
        Object.keys(this.selectedFilter).forEach((key) => {
            Object.keys(this.selectedFilter[key]).forEach((value) => {
                ...
                } else if (this.selectedFilter[key][value] &&
                           value == ALL_FILTER_ID &&
                           key != ALL_FILTER_ID &&
                           this.schema[key].include_to_default_filter) {
                    let first = true;
                    this.schema[key].values.forEach((item) => {
                        if (first) {
                            result += '&filter[]=' + key + '%3D' + item.value;
                            first = false;
                        } else {
                            result += ',' + item.value;
                        }
