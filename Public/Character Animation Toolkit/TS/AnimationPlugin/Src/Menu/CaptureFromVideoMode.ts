...
export class CaptureFromVideoMode {
    ...
    private selectCharacterPage: SelectCharacterPage;
    ...
    create(parent: Ui.Widget, onReturnCallback: Function, goToGalleryPage: Function): Ui.Widget {
        ...
        reloadButton.setIconWithMode(Editor.Icon.fromFile(import.meta.resolve('./Resources/refresh.svg')), Ui.IconMode.MonoChrome);
        ...
        const transparentIcon = Editor.Icon.fromFile(import.meta.resolve('./Resources/transparent.png'));
        ...
        const guidelinesLink = 'https://developers.snap.com/lens-studio/features/genai-suite/character-animation#capture-from-video';
        ...
        processVideoButton.onClick.connect(() => {
            if (this.currentVideoFilePath) {
                ...
                uploadAnimation(this.currentVideoFilePath as unknown as Editor.Path, fileName, (response: any) => {
                    if (!this.isActive) {
                        return;
                    }
