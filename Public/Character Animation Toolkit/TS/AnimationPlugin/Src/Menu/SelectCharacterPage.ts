...
export class SelectCharacterPage {
    ...
    private generateButton: Ui.PushButton | undefined;
    private isActive: boolean;
    private grid: Grid | undefined;
    private selectedCnt: number;
    ...
    create(parent: Ui.Widget, onReturnCallback: Function, goToGalleryPage: Function) {
        ...
        this.generateButton.setIconWithMode(Editor.Icon.fromFile(import.meta.resolve('./Resources/generate.svg')), Ui.IconMode.MonoChrome);
        ...
        this.connections.push(this.generateButton.onClick.connect(() => {
            ...
            this.grid.getVisibleTiles().forEach((tile, i) => {
                if (tile.visible && tile.selected && this.animationId) {
                    ...
                    proceedWithSelectedTrack(this.animationId, i, (response: any) => {
                        if (!this.isActive) {
                            return;
                        }
    ...
    addTracks(tracks: any) {
        ...
        tracks.forEach((track: any, i: number) => {
            ...
            this.assetLibImporter?.downloadAsset(track.previewUrl, "track_" + i + ".jpg", (response: any) => {
                if (!this.isActive) {
                    return;
                }
                if (this.grid && response.success) {
                    tile.background = new Ui.Pixmap(response.path);
    ...
    private createWarningDialog(onReturnCallback: Function) {
        const pluginSystem: PluginSystem = dependencyContainer.get(DependencyKeys.PluginSystem) as PluginSystem;
        const gui = pluginSystem.findInterface(Ui.IGui) as Ui.Gui;
