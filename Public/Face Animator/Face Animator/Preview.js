...
export class Preview {
    constructor(parent, onItemRemovedCallback, onTrainingStartedCallback) {
        this.connections = [];
        this.previews = {};
        this.id = "";
        this.previewId = 0;
        ...
        this.onItemRemovedCallback = () => { };
        this.onTrainingStartedCallback = () => { };
    ...
    createFooter(parent) {
        ...
        const _this = this;
        const importToProject = (dnnPath, audioPath) => {
            _this.downloadAsset(_this.outputModelUrl, _this.id + "_output.dnn", (path) => {
                dnnPath = path;
                if (audioPath !== null) {
                    if (audioPath === "failed") {
            ...
            if (audioPath === null) {
                _this.downloadAsset(_this.mp3Url, _this.id + "_audio.mp3", (path) => {
                    audioPath = path;
                    if (dnnPath !== null) {
                        if (audioPath === "failed") {
                            _this.importer.importToProject(dnnPath, null);
    ...
    setVideoPlayStartListener(callback) {
        this.onVideoPlayStartCallback = callback;
    }
    setVideoPlayStopListener(callback) {
    ...
    previewIdxChanged(id, previewId) {
        if (this.previews[id][previewId]) {
            ...
            this.downloadAsset(this.previewUrls[previewId].sourceImageUrl, id + "_" + previewId + "_sourceImage.webp", (path) => {
            ...
            this.downloadAsset(this.previewUrls[previewId].outputVideoUrl, id + "_" + previewId + "_outputVideo.mp4", (path) => {
    ...
    createWarningDialog(addAudioCallback, skipAudioCallback) {
