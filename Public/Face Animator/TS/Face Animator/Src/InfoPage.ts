...
export class InfoPage {
    ...
    onNewAnimatorCreated: Function = (animatorData: any) => {
        ...
        this.downloadPreview(animatorData.uploadUrl, animatorData.id + "_preview" + ".mp4", (path: Editor.Path) => {
            newItem.setPreview(path);
        })
    }

    private downloadPreview(url: Editor.Path, filename: string, callback: Function) {
