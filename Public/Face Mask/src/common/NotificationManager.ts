...
export enum NotificationKey {
    ErrorViolatesCommunityGuidelines,
    ErrorReachedLimit,
    ErrorUnknown,
    InfoImportSuccess,
    ErrorImportFailed,
...
export class NotificationManager {
    private mDisplayDuration: number;
    private mNotificationTimer: Timeout | null;

    notificationContainer: Ui.Widget;
    notificationLayout: Ui.BoxLayout;
    notificationWidgets: { [key: number | string]: ErrorNotificationWidget | InfoNotificationWidget } = {};
    ...
    private addNotificationWidget(key: NotificationKey, type: NotificationType, message: string, width: number = UIConfig.NOTIFICATION_BANNER.WIDTH) {
        if (!this.notificationContainer) {
    ...
    private configureNotification() {
        ...
        // import
        this.addNotificationWidget(NotificationKey.InfoImportSuccess, NotificationType.Info, 'Face Mask has been imported successfully.', 275);
        this.addNotificationWidget(NotificationKey.ErrorImportFailed, NotificationType.Error, 'Failed to import Face Mask, please try again.', 280);
    }

    set notificationDuration(durationInMilliseconds: number) {
    ...
    showNotification(key: NotificationKey) {
        const notificationWidget: Ui.Widget = this.notificationWidgets[key];
