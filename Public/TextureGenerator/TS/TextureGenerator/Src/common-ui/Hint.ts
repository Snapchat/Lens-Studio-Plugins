...
export function createHintIcon(
    parent: Ui.Widget,
    hintWidget: Ui.Widget,
    iconPath?: string,
    iconSize: number = Ui.Sizes.IconSide
): { icon: Ui.ImageView; connection: any } {
    ...
    const iconImage = new Ui.Pixmap(resolvedIconPath as any);
    ...
    const connection = infoToolTip.onHover.connect((hovered: boolean) => {
        if (hovered) {
...
export function createStandardHintWidget(
    parent: Ui.Widget,
    title: string,
    description: string,
    imagePath?: string
    ...
    if (imagePath) {
        const imagePixmap = new Ui.Pixmap(imagePath as any);
