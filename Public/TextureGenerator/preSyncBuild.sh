...
# ============================================================================
# preSyncBuild.sh -- Texture Generator CDS sync build script
#
...
#      folder) into build/resources/<version>/TextureGenerator.zip
#
# WHY FLAT (no wrapper):
#   Today Texture Generator has no custom preSyncBuild; CDS uses the repo-root
#   default script which zips the (previously committed) module folder's
#   CONTENTS flat -- entries at the zip root: Main.js, common-ui/..., module.json,
#   etc., with NO wrapper folder. This script reproduces that exact layout so the
#   produced package is byte-equivalent to today's live upload.
#
...
echo "=== Texture Generator: preSyncBuild.sh starting ==="
...
STAGE_DIR="${TMPDIR:-/tmp}/texture-generator-pkg-$$"
...
ZIP_DEST="$ZIP_DIR/TextureGenerator.zip"
...
echo "=== Texture Generator: preSyncBuild.sh completed ==="
