...
import { importToProject, createGenerationErrorWidget } from '../utils.js';
import { deleteAsset } from '../api.js';
...
export class AssetPreview {
    ...
    deleteAsset(id) {
        app.log('Deleting Gaussian Splatting asset...', { 'progressBar': true });
        ...
        deleteAsset(id, function(response) {
            ...
            if (response.statusCode == 204) {
                ...
                app.log('Gaussian Splatting Asset has been deleted.');
            } else {
                this.onStateChanged({
                    'screen': 'default',
                    'needsUpdate': true,
                ...
                app.log('Gaussian Splatting Asset could not be deleted. Please, try again.');
            }
        }.bind(this));
    }
    ...
    createPreview(parent) {
        ...
        this.previewImage.setFixedHeight(this.maxSize);
        this.previewImage.setFixedWidth(this.maxSize);
    ...
    setPreview(image) {
        this.previewImage.movie = image;

        const width = this.previewImage.movie.width;
        const height = this.previewImage.movie.height;
        ...
        this.previewImage.setFixedWidth(newWidth);
        this.previewImage.setFixedHeight(newHeight);
        ...
        if (state.status) {
            ...
            if (this.status === 'FAILED' || this.status === 'UNSAFE') {
                switch(state.errorMessage) {
                    case "UNSAFE_VIDEO":
                        this.setErrorMessage("Unsafe content detected, please try different video.");
                        app.log('This asset violates our community guidelines and should be deleted.', { 'enabled': true });
                    break;
                    case "COLMAP_ERROR":
                        this.setErrorMessage("Poor capture, please refer to our capture guidelines.");
                    break;
                    default:
                        this.setErrorMessage('<center>There was a problem generating the Gaussian Splatting asset.</center><center>Please try to generate another one.</center>');
                    break;
                }
                ...
                if (state.preview_image) {
                    this.setPreview(state.preview_image);
    ...
    createDeletionDialog() {
        ...
        headerLabel.text = 'Delete the Gaussian Splatting asset?';
        headerLabel.fontRole = Ui.FontRole.TitleBold;
        paragraphLabel.text = 'This will delete this Gaussian Splatting asset permanently. You cannot undo this action.';
    ...
    onImportClicked() {
        app.log('Importing Gaussian Splatting to the project...', { 'enabled': true, 'progressBar': true });
        try {
            importToProject(this.plyUrl, function(success) {
                logEventAssetImport(success ? "SUCCESS" : "FAILED");
                app.log(success ? 'Gaussian Splatting is successfully imported to the project' : 'Import failed, please try again');
            }.bind(this));
        } catch (error) {
            logEventAssetImport("FAILED");
            app.log('Gaussian Splatting import is failed. Please, try again');
        }
    }
    ...
    create(parent) {
        ...
        [this.errorWidget, this.setErrorMessage] = createGenerationErrorWidget(this.widget);

        this.stackedWidget.addWidget(this.errorWidget);
